...
class custom_oauth_client_class extends oauth_client_class
{
    var $service;
    var $session = '';
    var $user = 0;
    ...
    Function Query($sql, $parameters, &$results, $result_types = null)
    {
        ...
        {
            echo '['.$trace['file'].']: '.$trace['class'].'#'.$trace['function'].'('.$trace['line'].')<br/>';
        }
    ...
    {
        if(parent::GetStoredState($state))
            return true;
    ...
    Function StoreAccessToken($access_token)
    {
        ...
        if($media)
        {
            $media->setAccessToken($access_token);
        }
        return true;
    ...
    Function GetAccessToken(&$access_token)
    {
        ...
        if(parent::getAccessToken($access_token))
        {
